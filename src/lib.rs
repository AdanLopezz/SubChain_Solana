...
Todas las funciones que modifican el vector interno implementan la siguiente macro de validación antes de ejecutar la lógica central:
...
Create (Inicialización) - inicializar_gestor: Se deriva la PDA usando la semilla [b"gestor", owner.key()]. Asigna el owner, el nombre del usuario e inicializa el vector de suscripciones completamente vacío (Vec::new()).

Create (Inserción) - agregar_suscripcion: Recibe el nombre y el costo como parámetros. Automáticamente asigna el booleano activa: true. Instancia el struct Suscripcion y lo empuja al final del vector usando el método nativo de Rust .push().

Update (Modificación) - editar_suscripcion: Realiza una búsqueda iterativa mediante un bucle for. Compara el nombre recibido con los nombres existentes en el vector. Al encontrar la coincidencia, sobrescribe directamente el costo y el estado booleano (activa), permitiendo al usuario simular "pausas" en sus pagos.

Delete (Eliminación) - eliminar_suscripcion: Utiliza iteradores funcionales y cierres (closures) de Rust: .iter().position(|s| s.nombre == nombre). Si la búsqueda arroja un Some(index), utiliza el método .remove(index) para sacar el elemento del vector, lo que reorganiza la memoria interna automáticamente. Si no lo encuentra, devuelve un error personalizado SuscripcionNoEncontrada.

Read (Lectura) - ver_suscripciones: Utiliza la macro msg! con el formateador de depuración {:#?} para emitir un log On-Chain legible con todo el contenido del vector.
